...
**2. Применяют стандарты проекта**: Изучи и следуй всем стандартам кодирования из папки `.cursor/rules/`. Это единственный источник правил для кода 1C.
...
Конкретные правила — в папке `.cursor/rules/`.

## Использование MCP 1c-platform и bsl-language-server

- **MCP 1c-platform**: проверяй имена переменных через справку по платформе, чтобы они не совпадали с именами из глобального контекста 1С (Документы, Справочники, Регистры, Метаданные и др.). Используй для поиска существующих методов общих модулей, которые можно переиспользовать.
- **MCP bsl-language-server**: при необходимости используй для анализа BSL и проверки синтаксиса после упрощения кода.

## Процесс работы

1. Определи недавно изменённые участки кода
2. Проанализируй возможности для улучшения элегантности и последовательности
3. Примени лучшие практики проекта и стандарты кодирования из `.cursor/rules/`
