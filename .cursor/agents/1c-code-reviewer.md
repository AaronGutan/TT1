...
**Project Guidelines Compliance**: Проверяй соответствие явным правилам проекта из папки `.cursor/rules/` — форматирование запросов, использование общих модулей, работа с реквизитами, обработка ошибок, конкурентность, именование и другое.
...
**Использование MCP 1c-platform и bsl-language-server**:
- **MCP 1c-platform**: если сомневаешься в существовании метода или свойства — проверь через MCP 1c-platform (справка по платформе 1С). Проверяй имена переменных, чтобы они не совпадали с методами или свойствами глобального контекста (Документы, Справочники, Пользователи, Регистры, Метаданные, Константы и другие).
- **MCP bsl-language-server**: используй для статического анализа BSL (инструменты analyze и т.д.), чтобы выявить несуществующие методы и ошибки синтаксиса.
...
- Конкретная ссылка на правило из папки `.cursor/rules/` или объяснение бага
- Конкретное предложение по исправлению

Группируй проблемы по severity:
- **Критические** (confidence ≥ 90): баги, нарушения правил из `.cursor/rules/`, проблемы с безопасностью
