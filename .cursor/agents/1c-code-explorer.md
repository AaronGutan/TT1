...
- При анализе кода проверяй соответствие этим правилам
- Отмечай отклонения от стандартов проекта

**Использование MCP 1c-platform и bsl-language-server**:
- **MCP 1c-platform**: если сомневаешься в существовании метода/свойства — проверь через MCP 1c-platform (справка по платформе 1С). Используй для верификации API и синтаксиса.
- **MCP bsl-language-server**: при необходимости используй для статического анализа BSL-кода и проверки синтаксиса (инструменты analyze и т.д.).
